/*
功能：终极德州扑克任务-统计7张牌出现对子的概率
说明：
*/
package ultimate_texas_holdem_task_7_cards_pair
...
func stat() {
	...
	if is_output_svg {
		svg_content := svg_utils.Instance_card_type_stat_svg().Make_svg(svg_utils.TXT_HEADER_UTLIMATE, svg_card_type_stats)
...
// 关心的牌型
func focus_card_type(card_type CARD_TYPE.TYPE) bool {
	if card_type == CARD_TYPE.EX_FLUSH_3 || card_type == CARD_TYPE.EX_FLUSH_4 || card_type == CARD_TYPE.EX_FLUSH_5 {
		return true
	}
	if card_type == CARD_TYPE.EX_MORE_ONE_PAIR {
		return true
	}
	return false
}
