...
var (
	wg                 sync.WaitGroup
	stat_plo4_win_rate StatPlo4WinRate
)
...
func Start() {
	...
	//设置手牌
	stat_plo4_win_rate.play1_hole_cards = []byte{0x3B, 0x24, 0x33, 0x35}
	stat_plo4_win_rate.play2_hole_cards = []byte{0x13, 0x1A, 0x2C, 0x2A}
...
func run_unit() {
	...
	//玩家1手牌
	removed_cnt, remain_shoe_cards := algorithm.Removes(shoe_cards, stat_plo4_win_rate.play1_hole_cards)
	if removed_cnt != len(stat_plo4_win_rate.play1_hole_cards) {
		xlog_entry.Errorf("run_unit(),removed_cnt:%d,", removed_cnt)
		return
	}

	//玩家2手牌
	removed_cnt, remain_shoe_cards = algorithm.Removes(remain_shoe_cards, stat_plo4_win_rate.play2_hole_cards)
	if removed_cnt != len(stat_plo4_win_rate.play2_hole_cards) {
		xlog_entry.Errorf("run_unit(),removed_cnt:%d,", removed_cnt)
		return
	}
	...
	//组合最佳牌型
	play1_item := logic_plo.Combo_best_card_type(stat_plo4_win_rate.play1_hole_cards, board_cards)
	play2_item := logic_plo.Combo_best_card_type(stat_plo4_win_rate.play2_hole_cards, board_cards)

	//比较
	ct := logic.Compare(play1_item, play2_item)
	stat_plo4_win_rate.Set_result(ct)

	str_play1_hole_cards := common.Cards_2_sign(stat_plo4_win_rate.play1_hole_cards)
	str_play2_hole_cards := common.Cards_2_sign(stat_plo4_win_rate.play2_hole_cards)
...
// 统计
func stat() {
	xlog_entry.Infof("stat_plo4_win_rate:%s", stat_plo4_win_rate.String())
}
